...
pub fn build_data_model<'a, V: Visitor<'a>>(
  ...
  declarations.models.values().for_each(|model| {
    visit_model_for_update(v, &mut ctx, model);
  });
...
fn visit_model_for_update<'a, V: Visitor<'a>>(
  v: &mut V,
  ...
  model.fields.iter().for_each(|field| {
    ctx.with_model(model, |ctx| {
      visit_field_for_update(v, ctx, field);
    });
  });
...
fn visit_field_for_update<'a, V: Visitor<'a>>(
  v: &mut V,
  ctx: &mut VisitorContext<'a>,
  field: &'a FieldDecl,
) {
  ...
  let field_relation = ctx
    .input_declarations()
    .expect("Declarations should given for building data model")
    .models
    .get(&type_name);
