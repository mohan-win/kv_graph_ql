...
use crate::{
  graphql_parser::{parse_query, types::ExecutableDocument},
  graphql_value::Variables,
  schema::IntrospectionMode,
  Data, ParseRequestError, ServerError,
};

/// GraphQL request.
///
/// This can be deserialized from a structure of the query string, the operation
/// name and the variables. The names are all in `camelCase` (e.g. `operationName`).
#[non_exhaustive]
...
impl Request {
  ...
  /// Specify operation name of the request.
  #[must_use]
  ...
  /// Disable introspection for this request.
  #[must_use]
...
impl BatchRequest {
  /// Attempt to convert batch request into single request.
  ///
  /// # Errors
  ///
  /// Fails if the batch request is a list of request, with a message saying
  /// that batch request aren't supported.
  ...
  /// Returns iterator over the requests.
  ...
  /// Specify variables for each requests.
  #[must_use]
  ...
  /// Specify some data for each requests.
  #[must_use]
...
mod tests {
  use super::*;
  use graphql_value::{from_value, value, DeserializerError};
