    ...
    match registry::MetaTypeName::create(type_name) {
        ...
        registry::MetaTypeName::List(type_name) => match value {
            ConstValue::List(elems) => {
                elems.iter().enumerate().find_map(|(idx, elem)| {
                ...
                .unwrap_or_else(|| panic!("Type `{}` not defined", type_name))
            {
                ...
                registry::MetaType::Enum {
                    ...
                    ConstValue::Enum(name) => {
                        if !enum_values.contains_key(name.as_str()) {
                            Some(valid_error(
                                &path_node,
                                format!("enumeration type \"{}\" doesn't contain value \"{}\"", enum_name, name),
                            ))
                        } else {
                            None
                        }
                ...
                registry::MetaType::InputObject {
                    ...
                    ConstValue::Object(values) => {
                        if *oneof {
                            if values.len() != 1 {
                                return Some(valid_error(&path_node, "Oneof input objects required to have exactly one field.".to_string()));
                            }
                        ...
                        for field in input_fields.values() {
                            ...
                            } else if registry::MetaTypeName::create(&field.ty)
                                .is_non_null()
