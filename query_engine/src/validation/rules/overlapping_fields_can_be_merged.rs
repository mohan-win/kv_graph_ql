      ...
      match &selection.node {
        ...
        Selection::FragmentSpread(fragment_spread) => {
            ...
            self.ctx.fragment(&fragment_spread.node.fragment_name.node)
          {
            ...
            {
              // To avoid recursing iteself, this error is detected by
  ...
  fn add_output(
    ...
    if let Some(prev_field) = self.outputs.get(&(on_type, name)) {
      if prev_field.node.name.node != field.node.name.node {
        self.ctx.report_error(vec![prev_field.pos, field.pos], 
          format!(r#"Fields "{}" conflict because "{}" and "{}" are different fields. Use different alias on fields to fetch both if this was intentional."#,
            name, prev_field.node.name.node, field.node.name.node));
      }

      // check arguments
      if prev_field.node.arguments.len() != field.node.arguments.len() {
        self.ctx.report_error(vec![prev_field.pos, field.pos], format!(r#"Fields "{}" conflict because they have different arguments. Use different alias on the fields to fetch both if this was intentional."#,name));
      }

      for (name, value) in &prev_field.node.arguments {
        match field.node.get_argument(&name.node) {
          Some(other_value) if value == other_value => {},
          _ => self.ctx.report_error(vec![prev_field.pos, field.pos], format!(r#"Fields "{}" conflict because they have different arguments. Use different alias on the fields to fetch both if this was intentional."#, name)
          ),
        }
      }
    } else {
