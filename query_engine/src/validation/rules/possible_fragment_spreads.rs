...
impl<'a> Visitor<'a> for PossibleFragmentSpreads<'a> {
  ...
  fn enter_fragment_spread(
    ...
    if let Some(fragment_type) = self
      .fragment_types
      .get(fragment_spread.node.fragment_name.node.deref())
    {
      if let Some(current_type) = ctx.current_type() {
        if let Some(on_type) = ctx.registry.types.get(*fragment_type) {
          if !ctx.registry.type_overlap(current_type, on_type) {
            ctx.report_error(
              vec![fragment_spread.pos],
              format!(r#"Fragment "{}" cannot be spread as the object of type "{}" can never be of type "{}"."#, fragment_spread.node.fragment_name.node, current_type.name(), fragment_type),
            );
          }
        }
      }
    ...
    if let Some(parent_type) = ctx.parent_type() {
        ...
        .map(|c| &c.node)
      {
        if let Some(on_type) = ctx.registry.types.get(fragment_type.node.as_str()) {
          if !ctx.registry.type_overlap(parent_type, on_type) {
            ctx.report_error(
              vec![inline_fragment.pos],
              format!(r#"Fragment cannot be spread here as objects of type "{}" can never be of type "{}""#, parent_type.name(), fragment_type),
            );
          }
        }
      }
