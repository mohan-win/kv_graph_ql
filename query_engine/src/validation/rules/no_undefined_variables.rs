...
impl<'a> NoUndefinedVariables<'a> {
  fn find_undef_vars(
    ...
    if let Some(used_var) = self.used_variables.get(scope) {
      for (var, pos) in used_var {
...
impl<'a> Visitor<'a> for NoUndefinedVariables<'a> {
      ...
      for (var, pos) in undef {
        if let Some(op_name) = op_name {
          ctx.report_error(
            vec![*def_pos, pos],
            format!(
              r#"Variable "${}" is not defined by the operation "{}"."#,
              var, op_name
            ),
          );
        } else {
  ...
  fn enter_variable_definition(
    ...
    if let Some(Scope::Operation(ref name)) = self.current_scope {
      if let Some((_, ref mut vars)) = self.defined_variables.get_mut(name) {
